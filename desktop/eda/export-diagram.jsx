...
export function diagramSvg(
  ...
  view,
  style,
  { Scene = EdaScene, showCardinality = true, ...sceneProps } = {},
) {
  ...
  svg.setAttribute("width", String(Math.ceil(view[2])));
  svg.setAttribute("height", String(Math.ceil(view[3])));
...
export async function svgToPng(svgText, view) {
  const scale = Math.min(
    2,
    8192 / Math.max(view[2], view[3]),
    Math.sqrt(24_000_000 / (view[2] * view[3])),
  );
  const canvas = document.createElement("canvas");
  canvas.width = Math.max(1, Math.ceil(view[2] * scale));
  canvas.height = Math.max(1, Math.ceil(view[3] * scale));
  const image = new Image(),
    url = URL.createObjectURL(new Blob([svgText], { type: "image/svg+xml" }));
  try {
    await new Promise((resolve, reject) => {
      image.onload = resolve;
      image.onerror = () => reject(new Error("无法生成图片，请尝试 SVG 导出"));
      image.src = url;
    });
