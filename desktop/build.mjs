...
import { layoutCacheVersion } from './build/layout-cache-version.mjs';
...
await build({root:source,configFile:false,plugins:[{name:'er-wiki-desktop',enforce:'pre',transform(code,id){return localize(integrate(code,id)||code,id);}},react()],
  resolve:{alias:rendererAliases(here,source)},define:{__ER_LAYOUT_CACHE_VERSION__:JSON.stringify(layoutCacheVersion(here))},
