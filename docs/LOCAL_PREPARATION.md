...
fulfillment example. The upstream editor comes from the exact public drawDB
commit in upstream.json; only the sanitized patch is applied.

Completed locally:

- Renderer build, using existing dependency caches.
- Targeted source and built-JavaScript privacy checks.
- Targeted scan for private-workspace identifiers, with no matches in candidate
  source or renderer output.
- Git index checks excluding dependency-cache symlinks and generated directories.

An initial private-network pattern matched a numeric SVG path segment in an
upstream icon. The guard now requires a complete address token, rather than
